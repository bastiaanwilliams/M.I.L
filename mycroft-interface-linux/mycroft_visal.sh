...
last=`tail -$ln /var/log/mycroft/voice.log | grep "open website" |wc -l`
expr $last + 0
if [ $last -eq 1 ] ; then
website=`tail -$ln /var/log/mycroft/voice.log | grep Utterance | cut -c103-| tr -d '"]'| rev | cut -c 2- | rev`
website1="https://www.$website"
website=$website1
killall mpg123
if [ $notify -gt 0 ] ; then
echo "OPENENED WEBSITE" >> /var/log/mycroft/voice.log
echo "OPENENED WEBSITE" >> /var/log/mycroft/voice.log
else
echo "OPENENED WEBSITE" >> /var/log/mycroft/voice.log
fi
killall mpg123
xdg-open $website &
mycroft-speak "Opening website $website"
fi
last=`tail -$ln /var/log/mycroft/voice.log | grep "google" |wc -l`
expr $last + 0
if [ $last -eq 1 ] ; then
killall mpg123
website=`tail -$ln /var/log/mycroft/voice.log | grep Utterance | cut -c97-| tr -d '"]'| rev | cut -c 2- | rev`
killall mpg123
if [ $notify -gt 0 ] ; then
echo "SEARCHED FOR $website" >> /var/log/mycroft/voice.log
echo "SEARCHED FOR $website" >> /var/log/mycroft/voice.log
else
echo "SEARCHED FOR $website" >> /var/log/mycroft/voice.log
fi
...
killall mpg123
xdg-open "https://www.google.com/search?q=$website" &
mycroft-speak "Searching for $website"
fi
}
...
say: ''make some noise'' - will play radio stream, fixed stream url. You can change this in the main script.

say: ''kill noise'' - will stop playing the radio stream

say: ''check diskspace'' - will show disk space overview (df -h)

say: ''open website #website'' - opens the website in the default browser(tab). Only in window or NO INTERFACE mode.

say: ''google #whatever'' - open google search for #whatever in the default browser(tab). Only in window or NO INTERFACE mode.

say: ''write letter'' - opens LibreOffice. If not installed it does not work.
...
clear
choice=`zenity --list --radiolist --column Selection --column="Name of Theme" JFULL "J.A.R.V.I.S FULLSCREEN" JWIN "J.A.R.V.I.S WINDOWED" STAR-TREK "STAR-TREK" K.I.T.T "K.I.T.T" NONE "NO INTERFACE" --width=400 --height=500 --text "
...
kitt 
#NONE
elif [ "$choice" = "NO INTERFACE" ] ;then
                         echo "Starting M.I.L without interface.."
notify-send "M.I.L started in NON-VISUAL MODE" &
none
else
pkill -f  feh
pkill -f  pqiv
